...
public record VendorImageChangeCommand(
    string UserId,
    string Description,
    IFormFile? Image1,
    IFormFile? Image2,
...
public class VendorImageChangeCommandHandler : IRequestHandler<VendorImageChangeCommand, ErrorOr<Success>>
{
    private readonly IProfileRepository repository;
    private readonly IFileStorageService fileStorage;

    public VendorImageChangeCommandHandler(IProfileRepository repository, IFileStorageService fileStorage)
    {
        this.repository = repository;
        this.fileStorage = fileStorage;
    }

    public async Task<ErrorOr<Success>> Handle(VendorImageChangeCommand request, CancellationToken cancellationToken)
    {
        ...
        profile.VendorProfile.UpdateImages(image1Path ?? string.Empty, image2Path ?? string.Empty, request.Description, request.Instagram);
